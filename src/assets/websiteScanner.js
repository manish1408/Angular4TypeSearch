    ...
    html2canvas(body, {
     ...
     function findAllButtons() {
        ...
        for (var index = 0; index < inputControls.length; index++) {
            var element = inputControls[index];
            if (element.type == 'button' || element.type == 'submit') {
                allButtonsOnPage.push(element);
                buttonGrid.type = "Button";
                buttonGrid.value = element.value;
                buttonGrid.pageName = window.location.href.split('?')[0];
                $(element).css('border', 'solid 2px red');
                html2canvas(document.body, {
                    onrendered: function(canvas) {
                        buttonGrid.image = canvas.toDataURL();
                        
                    },
                    width: 300,
                    height: 300
                });
                jsonGrid.push(buttonGrid);
                $(element).css('border', 'initial');
            }
        }
        
        if (window.location.href === "http://localhost:3000/home") {
            var strGrid= JSON.stringify(jsonGrid);
            localStorage.setItem("evGrid", strGrid);
            console.log(localStorage.getItem("evGrid"));
            
            // window.location.href = 'http://localhost:3000/scanreport';
        }
    }
